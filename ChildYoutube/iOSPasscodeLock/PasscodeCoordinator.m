...
- (void)subscribeToNotifications {
    ...
    [[NSNotificationCenter defaultCenter] addObserver: self
                                             selector: @selector(handleNotification:)
                                               ...
                                               object: nil];*/
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(handleNotification:) name:TrackingInactivityStartedNotification object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(handleNotification:) name:TrackingInactivityEndedNotification object:nil];
}
...
- (void)disableSubscriptions {
    ...
    [[NSNotificationCenter defaultCenter] removeObserver:self name:TrackingInactivityStartedNotification object:nil];
    [[NSNotificationCenter defaultCenter] removeObserver:self name:TrackingInactivityEndedNotification object:nil];
}
...
- (void)handleNotification:(NSNotification *)notification {
    ...
    if ([notification.name isEqualToString:TrackingInactivityStartedNotification]) {
        [self dismissLockScreen];
        [self startTrackingInactivity];
        if ([self shouldLock]) {
            [self verifyPasscodeWithPasscodeType:PasscodeTypeVerify withCompletion:nil];
        }
    } else if ([notification.name isEqualToString:TrackingInactivityEndedNotification]) {
        [self stopTrackingInactivity];
        if ([self shouldLock]) {
            [self verifyPasscodeWithPasscodeType:PasscodeTypeVerify withCompletion:nil];
            [[NSNotificationCenter defaultCenter] postNotificationName:PasscodeLockedNotification object:self.passcodeWindow];
        }
    }
}
...
- (void)dismissLockScreen {
    if(self.passcodePresented) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:PasscodeDismissedNotification object:nil];
    }
    [self startTrackingInactivity];
}
